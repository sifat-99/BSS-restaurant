...
export const fetchFoods = createAsyncThunk(
    "food/fetchFoods",
    async ({ token, page = 1, perPage = 10, search = "" }: { token: string; page?: number; perPage?: number; search?: string }, { rejectWithValue }) => {
        try {
...
const foodSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(fetchFoods.rejected, (state:any, action:any) => {
                state.loading = false;
                state.error = action.payload || "Failed to fetch foods";
            })
            .addCase(createFood.fulfilled, (state:any, action:any) => {
                const newItem = action.payload?.id ? action.payload : { ...action.meta.arg.data, id: Date.now() };
                state.foods = [...state.foods, newItem];
                state.totalCount += 1;
            })
            .addCase(updateFood.fulfilled, (state:any, action:any) => {
                const { id, data } = action.meta.arg;
                const index = state.foods.findIndex((item:any) => item.id === id);
                if (index !== -1) {
                    state.foods[index] = { ...state.foods[index], ...data };
                }
            })
            .addCase(deleteFood.fulfilled, (state:any, action:any) => {
                const id = action.meta.arg.id;
                state.foods = state.foods.filter((item:any) => item.id !== id);
                state.totalCount -= 1;
            });
    },
});
