...
function DashboardMainPage({ window, children }: DashboardMainPageProps) {
    ...
    const getActiveTab = () => {
        const index = menuItems.findIndex((item) => {
            ...
            console.log(location.pathname, item.path, location.pathname.startsWith(item.path));
    ...
    const handleOpenBottomUserMenu = (event: React.MouseEvent<HTMLElement>) => {
        setAnchorElUser(event.currentTarget);
            ...
            <List>
                {menuItems.map((item) => (
                    <ListItem
                        ...
                        key={item.label} disablePadding sx={{
                            display: "block",
                            borderLeft: activeTab === menuItems.indexOf(item) ? `4px solid ${theme.palette.primary.main}` : "none",
                        }}>
                        <ListItemButton
